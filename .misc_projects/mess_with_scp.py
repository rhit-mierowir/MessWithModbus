...
if __name__ == "__main__":
    print("Run Tests")
    local_file=LocalPath(Path(Path(__file__).parent,"test_file.txt").resolve()) #Gets the file from the .misc_projects folder, not the current working directory.

    remote_user     = input("Remote User: ")
    remote_server   = input("Remote Server [e.g. 9.9.9.9]: ")
    remote_path     = input("Remote Path [e.g. ~/path/to/file]: ")
    #remote_user, remote_server, remote_path = "...", "...", "..."
    ssh_keypath = Path("~/.ssh/OT_Server_key").expanduser()

    remote_file=SCPAddress(f"{remote_user}@{remote_server}:{remote_path}",local_path=Path("./tmp_file"), ssh_key_path=ssh_keypath ,write_file_back=False)

    first_parse =parse_remotable_path("./test_file.txt",temporary_local_file=Path("./tmp"),write_back=False,ssh_key_path=ssh_keypath)
    second_parse=parse_remotable_path(f"{remote_user}@{remote_server}:{remote_path}",temporary_local_file=Path("./tmp"),write_back=False,ssh_key_path=ssh_keypath)

    with remotable_open(local_file,mode='r') as f:
        print("="*25+"[ local file ]"+"="*25)
        for line in f.readlines():
            print("\t"+line)
    with remotable_open(remote_file,mode='r') as f:
        print("="*25+"[ remote file ]"+"="*25)
        for line in f.readlines():
            print("\t"+line)
    with remotable_open(remote_file,mode='a') as f:
        print("="*25+"[ remote file write ]"+"="*25)
        f.write(input("What should I append to the file?"))
    with remotable_open(first_parse,mode='r') as f:
        print("="*25+"[ first_parse file ]"+"="*25)
        for line in f.readlines():
            print("\t"+line)
    with remotable_open(second_parse,mode='r') as f:
        print("="*25+"[ second_parse file ]"+"="*25)
        for line in f.readlines():
            print("\t"+line)
